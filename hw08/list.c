...
int push_back(struct list *listToAppendTo, struct person *data)
{
    ...
    {
        listToAppendTo -> tail -> next = node;
    }
    listToAppendTo -> tail = node;
...
int contains(struct list *listToSearch, struct person *data, struct person **dataOut)
{
    ...
    {
        if (!person_eq_name(currNode -> data, data))
        {
            *dataOut = currNode -> data;
            return 1;
        }
...
int pop_front(struct list *listToPopFrom, struct person **dataOut)
{
    ...
    struct list_node *node = listToPopFrom -> head;
    *dataOut = node -> data;
    listToPopFrom -> head = node -> next;
...
int pop_back(struct list *listToPopFrom, struct person **dataOut)
{
    ...
    int i;
    struct list_node *currNode = listToPopFrom -> head;
                 ...
                 struct person **dataOut)
{
    ...
    if (!person_eq_name(currNode -> data, data_to_remove))
    {
        *dataOut = currNode -> data;
    ...
    int i;
    struct list_node *prevNode = currNode;
    currNode = currNode -> next;
    for (i = 0; i < listToRemoveFrom -> size - 2; i++)
    {
        if (!person_eq_name(currNode -> data, data_to_remove))
        {
            *dataOut = currNode -> data;
    ...
    if (listToRemoveFrom -> size > 1 && !person_eq_name(currNode -> data, data_to_remove))
    {
        *dataOut = currNode -> data;
        prevNode -> next = currNode -> next;
        listToRemoveFrom -> tail = prevNode;
        listToRemoveFrom -> size -= 1;
        free(currNode);
        return 1;
    }
...
void empty_list(struct list *listToEmpty)
{
    if (listToEmpty != NULL && listToEmpty -> size != 0)
    {
        ...
        struct list_node *prevNode = listToEmpty -> head;
        for (i = 0; i < listToEmpty -> size; i++)
        {
            prevNode = currNode;
            currNode = currNode -> next;
            person_free(prevNode -> data);
            free(prevNode);
        }
        listToEmpty -> size = 0;
    }
...
int reverse(struct list *listToReverse)
{
    ...
    {
        struct list_node *prevNode = listToReverse -> head;
        struct list_node *currNode = prevNode -> next;
        ...
        struct list_node *head = listToReverse -> tail;
        listToReverse -> tail = listToReverse -> head;
        listToReverse -> tail -> next = NULL;
        listToReverse -> head = head;
    }
    return 1;
}
